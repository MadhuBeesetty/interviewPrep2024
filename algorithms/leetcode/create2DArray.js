...
const findMatrix = (nums) => {
  ...
  for (var i = 0; i < nums.length; i++) {
      for (var j = 0; i < output.length; j++) {
          if (output[j].indexOf(nums[i]) === undefined) {
...
//   while (Object.keys(frequencyMap).length > 0) {
//       const row = [];
//       const keys = Object.keys(frequencyMap); // Get current keys

//       for (const key of keys) {
//           row.push(Number(key)); // Add the key to the current row
//           frequencyMap[key]--;  // Decrease the frequency of the key
