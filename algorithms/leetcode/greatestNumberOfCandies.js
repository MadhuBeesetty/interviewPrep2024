...
const kidsWithCandies = (candies, extraCandies) => {
  let output = new Array(candies.length).fill(true);

  for (var i = 0; i < candies.length; i++) {
      let compareval = candies[i] + extraCandies;
      console.log(i, candies[i], compareval, candies.length)
      for (var j = 0; j < candies.length; j++) {
          if (compareval < candies[j]) {
              output[i] = false;
              // console.log(i,j, candies[j], compareval, output);
              break;
          }
      };
  }
