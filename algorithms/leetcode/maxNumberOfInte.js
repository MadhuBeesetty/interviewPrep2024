...
// some number of integers following the below rules:
...
const maxCount = (banned, n, maxSum) => {
  ...
  for (var i = 1; i <= n; i++) {
      if (banned.indexOf(i) === -1) {
          if (arrSum(output) + i < maxSum) {
              output.push(i);
              console.log(output, arrSum(output));
          }
      }
  }
  return output;
...
const maxCount = (banned, n, maxSum) => {
  ...
  for (var i = 1; i <= n; i++) {
      if (banned.indexOf(i) === -1) {
          if (output[output.length - 1] + i < maxSum) {
              output.push(output[output.length - 1] + i);
              outputCount++
          }
      }
  }
